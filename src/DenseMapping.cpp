...
void fullydense_mapping(DenseMapping *pdense_mapper,ros::Publisher *pub_cloud)
{
    if (pdense_mapper->get_do_dense()> 0.5)
    {
        ...
        out_points.header.frame_id = "surface_reconstruction/map";
        ...
        if (points_superpixels_all.rows > 10)
        {
            ...
            char buffer_sup[150];
            sprintf (buffer_sup,(ros::package::getPath("surface_reconstruction")+"/src/map_and_poses/sup%d.ply").c_str(), pdense_mapper->dense_kf);
...
void calculate_superpixels_and_setdata(Imagenes &images,  int reference_image)
{
    ...
    chdir((ros::package::getPath("surface_reconstruction")+"/ThirdParty/segment").c_str());
